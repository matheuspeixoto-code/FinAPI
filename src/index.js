...
app.post("/account", (request,response)=>{
    ...
    const customerAlreadyExists = customers.some((custome)=> custome.cpf === cpf);
...
app.post("/withdraw",verifyIfExistAccountCPF,(request,response)=>{
    ...
    if(balance<amount){
        return response.status(400).json({error:"Impossível realizar a operação! Sáldo insuficiente"})
    }
